...
class AIACIMP(IAgent.IAgent):
    name = "AIACIMP_"

    def __init__(self, name, continueTraining=False, demonstrations="", initialEpsilon=1, loadNetwork="", saveFolder="",
                 verbose=False):
        ...
        fileNameDataSource = fileName + os.path.sep + "Datasource" + os.path.sep + "data.npy"
    ...
    def updateDemonstrations(self):
        if not self.demonstrations == None:
            while self.demonstrations.shape[0] < self.batchSize:
                ...
                self.demonstrations = numpy.append(self.demonstrations, numpy.expand_dims(self.demonstrations[0], 0),
                                                   axis=0)
    ...
    def huber_loss(y_true, y_pred):
        ...
        return tf.where(K.less_equal(K.abs(y_true - y_pred), delta), huber_mse, huber_mae)

    def compileModels(self):
        self.actor.compile(loss=self.loss, optimizer=Adam(lr=self.learning_rate), metrics=["mse"])

        self.targetNetwork.compile(loss=self.huber_loss, optimizer=Adam(lr=self.learning_rate), metrics=["mse"])
    ...
    def getOptmizer(self):
        ...
        self.updateOnline = K.function([state, nextState, actionProb, state_d, nextState_d, actionProb_d], loss,
    ...
    def updateModel(self, game, thisPlayer):
        ...
        if self.verbose:
            print("-- " + self.name + ": Epsilon:" + str(self.epsilon) + " - Loss Policy: " + str(
                lossPolicy) + " - Loss Reward: " + str(lossReward))
